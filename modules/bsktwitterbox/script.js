...
$(document).ready(function() {
    var footer = $('#footer');
    if(footer.has('#footerBar').length){
        footer.css('border-top', 'none');
        footer.css('padding-top', '0px');
        
        footer.children('.container').css('padding-top', '30px');
    }

    var bsktwitterbox = $('#bsktwitterbox');
    var widget_id = bsktwitterbox.data('widgetid');
    ...
    twitterFetcher.fetch(
        ...
        function(tweets){
            ...
            output += '</ul>'
            bsktwitterbox.append(output);
            
            bsktwitterbox.children('ul').carouFredSel({
                auto: {
                    timeoutDuration: 3500,
                    pauseOnHover: true
                },
                direction: 'right',
                prev: '#bsktwitterbox_right_arrow',
                next: '#bsktwitterbox_left_arrow',
            });
            
            var carousel_width = bsktwitterbox.children('.caroufredsel_wrapper').width();
            bsktwitterbox.find('p').width(carousel_width);
        }
    );
