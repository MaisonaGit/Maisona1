...
class BskSlide extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'bskslider_slides',
		'primary' => 'id_bskslider_slides',
		'multilang' => true,
		'fields' => array(
            ...
            'url'           =>  array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isCleanHtml', 'size' => 4000),
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'bskslider` (`id_shop`, `id_bskslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'bskslider`
			WHERE `id_bskslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_bskslider_slides` = hs.`id_bskslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_bskslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'bskslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'bskslider` hs ON (hss.`id_bskslider_slides` = hs.`id_bskslider_slides`)
