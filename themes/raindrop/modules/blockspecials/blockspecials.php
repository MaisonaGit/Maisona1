...
class BlockSpecialsBSK extends BlockSpecials{

    public function hookHomeNFS($params){
        if (Configuration::get('PS_CATALOG_MODE')) return ;
        $specials = Product::getPricesDrop((int)($params['cookie']->id_lang), 0, 8);
        if (!$specials && !Configuration::get('PS_BLOCK_SPECIALS_DISPLAY')) return;
        ...
        if( class_exists('ProductComment') ){
            ...
            if( count($avgStars) == count($specials) ){
