...
<div id="wishlist_block_top">
	<div class="title_block">
            <a href="{if isset($wishlist_link)}{$wishlist_link}{else}#{/if}" title="{l s='Wishlist' mod='blockwishlist'}">&nbsp;</a>
	</div>
            ...
            {if $wishlist_products}
                    ...
                    {foreach from=$wishlist_products item=product name=i}
                            <dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
                                    ...
                                    href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category_rewrite)}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|truncate:32:'...'|escape:'htmlall':'UTF-8'}</a>
