...
{if $products}
	...
	<div class="wishlistLinkTop">
		...
		<ul class="clearfix display_list">
			...
			{/if}
                ...
                <p class="wishlisturl">Permalink : <input class="form-control" type="text" value="{$base_dir_ssl}modules/blockwishlist/view.php?token={$token_wish|escape:'htmlall':'UTF-8'}" style="width:540px;" readonly="readonly" /></p>
		<p class="submit">
	...
	{/if}
	<div class="wlp_bought">{*$products|d*}
			...
			<li id="wlp_{$product.id_product}_{$product.id_product_attribute}" class="clearfix address {if $smarty.foreach.i.index % 2}alternate_{/if}item">
				...
				<div class="clearfix">
					...
					<div class="product_infos">
                                            <p class="product_name">{$product.name|truncate:30:'...'|escape:'htmlall':'UTF-8'}</p>
                                            <div class="cc-price">{convertPrice price=$product.price}</div>
                                            ...
                                            {/if}
                                            <div class="quantity">{l s='Quantity' mod='blockwishlist'}:<input class="form-control" type="text" id="quantity_{$product.id_product}_{$product.id_product_attribute}" value="{$product.quantity|intval}" size="3"  /></div>
                                            <div class="priority">
                                        ...
                                        <div class="btn_action">
                                            ...
                                            {if ($product.available_for_order && $product.stock_qty > 0)}
                                            <a class="btn wishlist_add_to_cart_button exclusive_small" data-id_product="{$product.id_product|intval}" data-id_combination="{$product.id_product_attribute}" data-quantity="{$product.quantity|intval}" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}", false)}" title="{l s='Add to cart'}">{l s='Add to cart'}</a>
                                            {/if}
                                        </div>
				</div>
                            </div>
	...
	<form method="post" class="wl_send std hidden" onsubmit="return (false);">
            <fieldset class="form-horizontal">
                <div class="form-group required">
                    <label for="email1" class="col-lg-4 control-label">{l s='Email' mod='blockwishlist'}1 <sup>*</sup></label>
                    <div class="col-lg-5">
                ...
                <div class="form-group">
                    <label class="col-lg-4 control-label" for="email{$smarty.section.i.index}">{l s='Email' mod='blockwishlist'}{$smarty.section.i.index}</label>
                    <div class="col-lg-5">
