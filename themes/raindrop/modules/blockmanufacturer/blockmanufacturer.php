...
class BlockManufacturerBSK extends BlockManufacturer{
    public function getContent(){
        $output = '<h2>'.$this->displayName.'</h2>';
        if (Tools::isSubmit('submitBlockManufacturers'))
        {
            $home_nb = (int)(Tools::getValue('home_nb'));
            ...
            {
                Configuration::updateValue('MANUFACTURER_DISPLAY_HOME_NB', $home_nb);
    ...
    public function displayForm(){
        ...
        <form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
                <fieldset><legend><img src="'.$this->_path.'logo.gif" alt="" title="" />'.$this->l('Settings').'</legend>
                        <label>'.$this->l('Home manufacturers carousel items').'</label>
                        <div class="margin-form">
                                <input type="text" size="2" name="home_nb" value="'.(int)(Tools::getValue('home_nb', Configuration::get('MANUFACTURER_DISPLAY_HOME_NB'))).'" />
                        </div>
    ...
    public function hookHome($params){
        $this->smarty->assign(array(
                ...
                'home_nb' => Configuration::get('MANUFACTURER_DISPLAY_HOME_NB'),
                'display_link_manufacturer' => Configuration::get('PS_DISPLAY_SUPPLIERS'),
        ));
        return $this->display(__FILE__, 'blockmanufacturer_home.tpl');
    }
    
    public function hookHeader($params){
        $this->context->controller->addCSS(($this->_path).'blockmanufacturer.css', 'all');
        $this->context->controller->addJS(($this->_path).'blockmanufacturer.js');
    }
}
