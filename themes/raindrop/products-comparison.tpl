...
<div class="frame_wrap frame_wrap_header clearfix">
    <div class="fwh-title">{l s='Product Comparison'}</div>
</div>
            ...
            <td width="{$width}%" class="ajax_block_product comparison_infos">
                    ...
                    {if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
                            {if ($product->quantity > 0 OR $product->allow_oosp)}
                                    <a class="btn exclusive ajax_add_to_cart_button" rel="ajax_id_product_{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")}" title="{l s='Add to cart'}"><span></span>{l s='Add to cart'}</a>
                            {else}
                                    <span class="btn exclusive">{l s='Add to cart'}</span>
                            {/if}
                    {else}
                            <div style="height:23px;"></div>
                    {/if}
                    <a class="cmp_remove btn button" href="{$link->getPageLink('products-comparison', true)}" rel="ajax_id_product_{$product->id}">{l s='Remove'}</a>
                </div>
            </td>
    {/foreach}
    </tr>
...
<div class="frame_wrap frame_wrap_content">
    <div class="alert alert-warning">
        <strong>Warning!</strong> {l s='There are no products selected for comparison.'}
    </div>
</div>
{/if}
