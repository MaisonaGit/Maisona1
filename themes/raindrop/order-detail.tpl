...
<form action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
    <div>
        ...
        <p class="title_block">
            <input type="submit" value="{l s='Reorder'}" name="submitReorder" class="btn button exclusive" />
...
<div class="adresses_bloc clearfix">
    ...
    <div class="col-lg-6 col-md-6 col-xs-12">
        <ul class="address item alert alert-info {if $order->isVirtual()}full_width{/if}">
    ...
    <div class="col-lg-6 col-md-6 col-xs-12">
        <ul class="address alternate_item alert alert-info" {if $order->isVirtual()}style="display:none;"{/if}>
...
<div id="order-detail-content" class="table_block clearfix">
	<table class="std table-vertical">
		<thead>
			<tr>
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
                                    <tr class="item">
                                        ...
                                        <td>
                                            <div class="th">{l s='Reference'}</div>
                                            <div class="td">
                                        ...
                                        <td>
                                            ...
                                            <div class="td">
                                                <input class="order_qte_input form-control"  name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$product.customizationQuantityTotal|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$product.customizationQuantityTotal|intval}</span></label>
                                            </div>
                                        </td>
                                        {if $order->hasProductReturned()}
                                        <td>
                                            <div class="th">{l s='Returned'}</div>
                                            <div class="td">
                                        ...
                                        {/if}
                                        <td>
                                            <div class="th">{l s='Unit price'}</div>
                                            <div class="td">
                                    ...
                                    {foreach $product.customizedDatas  as $customizationPerAddress}
                                            ...
                                            <tr class="alternate_item">
                                                    ...
                                                    <td>
                                                            <input class="order_qte_input form-control" name="customization_qty_input[{$customizationId|intval}]" type="text" size="2" value="{$customization.quantity|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$customization.quantity|intval}</span></label>
                                                    </td>
                                                    <td colspan="2"></td>
                                            </tr>
                                            {/foreach}
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
                                    <tr class="item">
                                        ...
                                        <td>
                                            <div class="th">{l s='Quantity'}</div>
                                            <div class="td">
                                                <input class="order_qte_input form-control" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$productQuantity|intval}</span></label>
                                            </div>
                                        </td>
                                        {if $order->hasProductReturned()}
                                        <td>
		...
		{/foreach}
                ...
                <tfoot>
                    {if $priceDisplay && $use_tax}
                        <tr class="item">
                            <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                                <div class="th">{l s='Total products (tax excl.)'}</div>
                                <div class="td">
                    ...
                    <tr class="item">
                        <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                            <div class="th">{l s='Total products'}</div>
                            <div class="td">
                                {if $use_tax}{l s='(tax incl.)'}{/if}: <span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithTaxes() currency=$currency}</span>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr class="item">
                        <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                            <div class="th">{l s='Total vouchers:'}</div>
                            <div class="td">
                    ...
                    <tr class="item">
                        <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                            <div class="th">{l s='Total gift wrapping cost:'}</div>
                            <div class="td">
                    ...
                    <tr class="item">
                        <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                            <div class="th">{l s='Total shipping'}</div>
                            <div class="td">
                                {if $use_tax}{l s='(tax incl.)'}{/if}: <span class="price-shipping">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
                            </div>
                        </td>
                    </tr>
                    <tr class="totalprice item">
                        <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                            <div class="th">{l s='Total'}</div>
                            <div class="td">
	...
	<div id="returnOrderMessage">
		...
		<p class="submit">
			<input type="submit" value="{l s='Make an RMA slip'}" name="submitReturnMerchandise" class="btn button" />
...
{if !$is_guest}
	...
	{if isset($message_confirmation) && $message_confirmation}
            <div class="alert alert-success">{l s='Message successfully sent'}</div>
	{/if}
	<form action="{$link->getPageLink('order-detail', true)|escape:'html'}" method="post" class="std form-horizontal" id="sendOrderMessage">
		<h3>{l s='Add a message'}</h3>
		<p>{l s='If you would like to add a comment about your order, please write it in the field below.'}</p>
                <div class="form-group">
                    <label class="col-lg-2 col-sm-2 control-label" for="id_product">{l s='Product'}</label>
                ...
                <div class="submit">
                    ...
                    <input type="submit" class="btn exclusive button" name="submitMessage" value="{l s='Send'}"/>
                </div>
	</form>
{else}
