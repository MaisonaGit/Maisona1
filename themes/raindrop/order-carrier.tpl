...
<div class="frame_wrap frame_wrap_header clearfix">
    <div class="fwh-title">{l s='Shipping:'}</div>
</div>
{else}
<div class="frame_wrap frame_wrap_header clearfix">
    <div class="fwh-title"><span>2</span> {l s='Delivery methods'}</div>
</div>
{/if}
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		{if !$is_guest}
			{if $back}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&back={$back}&multi-shipping={$multi_shipping}")}" title="{l s='Previous'}" class="btn button">&laquo; {l s='Previous'}</a>
			{else}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&multi-shipping={$multi_shipping}")}" title="{l s='Previous'}" class="btn button">{l s='Previous'}</a>
			{/if}
		{else}
				<a href="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")}" title="{l s='Previous'}" class="btn button">{l s='Previous'}</a>
		{/if}
		{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
			<input type="submit" name="processCarrier" value="{l s='Next'}" class="btn button exclusive" />
		{/if}
	</p>
</form>
{else}
	...
	<div>
		...
		<p><textarea class="form-control" cols="120" rows="3" name="message" id="message">{if isset($oldMessage)}{$oldMessage|escape:'htmlall':'UTF-8'}{/if}</textarea></p>
	</div>
</div>
{/if}
</div>
