...
function reloadContent(params_plus)
{
	...
	if (!ajaxLoaderOn)
	{
		$('#product_wrapper').parent().prepend($('#layered_ajax_loader').html());
		$('#product_wrapper').css('opacity', '0.5');
		ajaxLoaderOn = 1;
	}
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			$('.category-product-count').html(result.categoryCount);
                        ...
                        $('#product_wrapper').parent().children('.loading:first').remove();
			if (result.productList){
                            $('#product_wrapper').replaceWith(utf8_decode(result.productList));
                            if( $.cookie('view') == "list" ) {
                                $('#view_nav .list').click();
                            } else {
                                $('#view_nav .grid').click();
			...
			$('#product_wrapper').css('opacity', '1');
			if ($.browser.msie) // Fix bug with IE8 and aliasing
				$('#product_wrapper').css('filter', '');
...
function updateProductUrl()
{
	...
	if (typeof(param_product_url) != 'undefined') {
		$.each($('ul#product_wrapper li.ajax_block_product .product_img_link,'+
				'ul#product_wrapper li.ajax_block_product h3 a,'+
				'ul#product_wrapper li.ajax_block_product .product_desc a,'+
				'ul#product_wrapper li.ajax_block_product .lnk_view'), function() {
