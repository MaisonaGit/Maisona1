		...
		else
		{
			...
			for ($i = 0; $i < sizeof($products); ++$i)
			{
				$obj = new Product((int)($products[$i]['id_product']), false, $context->language->id);
                                $products[$i]['price'] = $obj->getPrice(true, $products[$i]['id_product_attribute']);
                                $products[$i]['available_for_order'] = $obj->available_for_order;
                                $products[$i]['stock_qty'] = StockAvailable::getQuantityAvailableByProduct($products[$i]['id_product'], $products[$i]['id_product_attribute']);
