...
{if $new_products !== false}
        ...
        <div class="item col-lg-3 col-sm-4 col-xs-6">
            <div class="cc-product">
                <a href="{$product.link}" title="{$product.name|escape:html:'UTF-8'}" class="product_image">
                    ...
                    {if isset($product.new) && $product.new == 1}<span class="new">{l s='New' mod='blocknewproducts'}</span>{/if}
                ...
                <div class="cc-foot">
                    ...
                    {if $product.show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
                        <div class="cc-price">
                            ...
                            {if !$priceDisplay}
                                {if $product.price_without_reduction > $product.price}{displayWtPrice p=$product.price_without_reduction}{/if}
                            {else}
                                {if $product['price_without_reduction'] > $product.price}{displayWtPrice p=$product['price_without_reduction']}{/if}
                            {/if}
                            </span>
            ...
            <div class="cc-product-hover">
                ...
                <a href="#" class="add_wishlist_button" onclick="WishlistCart('wishlist_block_top_list', 'add', '{$product.id_product|intval}', {$product.id_product_attribute|intval}, 1); return false;" title="{l s='Add to my wishlist' mod='blocknewproducts'}">{l s='Add to my wishlist' mod='blocknewproducts'}</a>
                {/if}
                <a href="{$product.link}" title="{$product.name|escape:html:'UTF-8'}" class="product_image">
                    <img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'home_default')}" height="{$homeSize.height}" width="{$homeSize.width}" alt="{$product.name|escape:html:'UTF-8'}" />
                    {if isset($product.new) && $product.new == 1}<span class="new">{l s='New' mod='blocknewproducts'}</span>{/if}
                ...
                <div class="cc-foot">
                    ...
                    {if ($product.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product.available_for_order AND !isset($restricted_country_mode) AND $product.minimal_quantity == 1 AND $product.customizable != 2 AND !$PS_CATALOG_MODE}
                        ...
                        <a class="btn exclusive_small ajax_add_to_cart_button" rel="ajax_id_product_{$product.id_product}" href="{$link->getPageLink('cart')}?qty=1&amp;id_product={$product.id_product}&amp;token={$static_token}&amp;add" title="{l s='Add to cart' mod='blocknewproducts'}">{l s='Add to cart' mod='blocknewproducts'}</a>
                        {else}
                        <span class="btn exclusive_small">{l s='Add to cart' mod='blocknewproducts'}</span>
                        {/if}
                    {/if}
    ...
    <center><a href="{$link->getPageLink('new-products')}" title="{l s='All new products' mod='blocknewproducts'}" class="btn button">{l s='All new products' mod='blocknewproducts'}</a></center>
{else}
