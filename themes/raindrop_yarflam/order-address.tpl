...
{if !$opc}
    <div class="fwh-title">{l s='Addresses'}</div>{else}<div class="fwh-title"><span>1</span> {l s='Addresses'}</div>
{/if}
</div>
...
{/if}
	<div class="addresses clearfix">
		<div class="address_delivery select clearfix">
			<label for="id_address_delivery" class="col-lg-4 col-lg-offset-1 col-sm-4">{if $cart->isVirtualCart()}{l s='Choose a billing address:'}{else}{l s='Choose a delivery address:'}{/if}</label>
		...
		<div class="addressesAreEquals clearfix" {if $cart->isVirtualCart()}style="display:none;"{/if}>
		...
		<div id="address_invoice_form" class="select clearfix" {if $cart->id_address_invoice == $cart->id_address_delivery}style="display: none;"{/if}>

		{if $addresses|@count > 1}
                    <label for="id_address_invoice" class="col-lg-4 col-lg-offset-1 col-sm-4">{l s='Choose a billing address:'}</label>
                    <div class="col-lg-4 col-sm-4>
			...
			{else}
				<a style="margin-left: 221px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button btn exclusive">{l s='Add a new address'}</a>
			{/if}
		</div>
                <div class="col-lg-6 col-md-6 col-xs-12">
                    <ul class="clearfix alert alert-info address item col-lg-6 col-md-6 col-xs-12" id="address_delivery" {if $cart->isVirtualCart()}style="display:none;"{/if}></ul>
                </div>
                <div class="col-lg-6 col-md-6 col-xs-12">
                    <ul class="clearfix alert alert-info address alternate_item col-lg-6 col-md-6 col-xs-12 {if $cart->isVirtualCart()}full_width{/if}" id="address_invoice"></ul>
                </div>
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button btn exclusive">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
		<div id="ordermsg" class="clearfix">
			...
			<p class="textarea"><textarea class="form-control" cols="60" rows="3" name="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea></p>
		</div>
		{/if}
	</div>
{if !$opc}
	<p class="cart_navigation submit">
		...
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}" title="{l s='Previous'}" class="btn button">{l s='Previous'}</a>
		<input type="submit" name="processAddress" value="{l s='Next'}" class="btn button exclusive" />
	</p>
</form>
{else}
</div>
