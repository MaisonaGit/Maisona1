...
<div class="frame_wrap frame_wrap_header clearfix">
    <div class="fwh-title">{l s='Shopping-cart summary'}</div>
</div>
...
{else}
	...
	<div style="display:none" id="emptyCartWarning" class="alert">{l s='Your shopping cart is empty.'}</div>
					...
					<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" class="product_customization_for_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval}{if $odd} odd{else} even{/if} customization alternate_item {if $product@last && $customization@last && !count($gift_products)}last_item{/if}">
						...
						<td class="cart_quantity" colspan="2">
							...
							{else}
								...
								<input size="2" type="text" value="{$customization.quantity}" class="cart_quantity_input form-control" name="quantity_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}"/>
							{/if}
						</td>
		...
		{if $use_taxes}
			{if $priceDisplay}
				<tr class="cart_total_price">
					<td class="phone-title" colspan="5">{if $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Total products'}{/if}</td>
					<td class="price phone-content" id="total_product" colspan="2">{displayPrice price=$total_products}</td>
				</tr>
			{else}
				<tr class="cart_total_price">
					<td class="phone-title" colspan="5">{if $display_tax_label}{l s='Total products (tax incl.)'}{else}{l s='Total products'}{/if}</td>
					<td class="price phone-content" id="total_product" colspan="2">{displayPrice price=$total_products_wt}</td>
				</tr>
			{/if}
		{else}
			<tr class="cart_total_price">
				<td class="phone-title" colspan="5">{l s='Total products'}</td>
				<td class="price phone-content" id="total_product" colspan="2">{displayPrice price=$total_products}</td>
			</tr>
		{/if}
			...
			{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
				<tr class="cart_total_delivery" style="{if !isset($carrier->id) || is_null($carrier->id)}display:none;{/if}">
					<td class="phone-title" colspan="5">{l s='Shipping'}</td>
					<td class="price phone-content" id="total_shipping" colspan="2">{l s='Free Shipping!'}</td>
				</tr>
			{else}
				{if $use_taxes}
					{if $priceDisplay}
						<tr class="cart_total_delivery" {if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
							<td class="phone-title" colspan="5">{if $display_tax_label}{l s='Total shipping (tax excl.)'}{else}{l s='Total shipping'}{/if}</td>
							<td class="price phone-content" id="total_shipping" colspan="2">{displayPrice price=$total_shipping_tax_exc}</td>
						</tr>
					{else}
						<tr class="cart_total_delivery"{if $total_shipping <= 0} style="display:none;"{/if}>
							<td class="phone-title" colspan="5">{if $display_tax_label}{l s='Total shipping (tax incl.)'}{else}{l s='Total shipping'}{/if}</td>
							<td class="price phone-content" id="total_shipping" colspan="2">{displayPrice price=$total_shipping}</td>
						</tr>
					{/if}
				{else}
					<tr class="cart_total_delivery"{if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
						<td class="phone-title" colspan="5">{l s='Total shipping'}</td>
						<td class="price phone-content" id="total_shipping" colspan="2">{displayPrice price=$total_shipping_tax_exc}</td>
					</tr>
				{/if}
			{/if}
			...
			<tr class="cart_total_price">
				<td class="phone-title" colspan="5">{l s='Total (tax excl.)'}</td>
				<td class="price phone-content" id="total_price_without_tax" colspan="2">{displayPrice price=$total_price_without_tax}</td>
			</tr>
			<tr class="cart_total_tax">
				<td class="phone-title" colspan="5">{l s='Total tax'}</td>
				<td class="price phone-content" id="total_tax" colspan="2">{displayPrice price=$total_tax}</td>
			</tr>
			{/if}
				...
				{if $voucherAllowed}
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
						<fieldset class="form-horizontal">
							<p class="title_block"><label for="discount_name" class="control-label">{l s='Vouchers'}</label></p>
							<div class="discount_name_block">
								<input type="text" class="discount_name form-control" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
							</div>
							<p class="submit"><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='OK'}" class="btn button exclusive" /></p>
						</fieldset>
					</form>
                                    </div>
                                    <div class="clearfix">
...
<p class="cart_navigation">
	{if !$opc}
		<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" class="btn button exclusive standard-checkout" title="{l s='Next'}">{l s='Next'}</a>
		{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
			<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1" class="multishipping-button multishipping-checkout btn button exclusive" title="{l s='Next'}">{l s='Next'}</a>
		{/if}
	{/if}
	<a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="button btn" title="{l s='Continue shopping'}">{l s='Continue shopping'}</a>
</p>
